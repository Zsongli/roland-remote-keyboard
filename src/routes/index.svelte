<script lang="ts">
	...
	var buttons = { w: false, a: false, s: false, d: false, e: false };

	$: init && roland.servo(servo);
	$: init && roland.LED(led);
	$: init && roland.buzzer(buttons.e ? 69 : 100);

	var move = { left: 0, right: 0 };
	$: if (init) {
		move = { left: 0, right: 0 };
		...
		if (buttons.a && !buttons.d) {
			if (!buttons.w && !buttons.s) move.left += speed;
			move.right = Math.ceil(move.right * 0.75);
		}

		if (buttons.d && !buttons.a) {
			if (!buttons.w && !buttons.s) move.right += speed;
			else move.left = Math.ceil(move.left * 0.75);
		}
...
<svelte:window
	on:keydown={(e) => {
		if (init && Object.keys(buttons).includes(e.key)) buttons[e.key] = true;
	}}
	on:keyup={(e) => {
		if (init && Object.keys(buttons).includes(e.key)) buttons[e.key] = false;
	}}
/>

{#await roland.init("http://192.168.0.1:5000").then(() => {
	init = true;
})}
	<div
		...
		<div class="border rounded-md p-5 border-neutral shadow-xl bg-base-100">
			...
			<div class="flex items-center-justify-evenly w-full gap-4">
				...
				<input
					...
					min="-90"
					max="90"
					class="range range-primary w-max pt-1"
					bind:value={servo}
				/>
				<span class="font-light">90°</span>
			</div>
...
{:catch error}
	<div
		class="animate-none btn btn-error btn-disabled btn-lg absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"
	>
		{error.message}
	</div>
{/await}
